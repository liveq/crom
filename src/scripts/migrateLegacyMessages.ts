/**
 * 성남시 사이트 추모 메시지 마이그레이션 스크립트
 * 백업된 HTML 파일에서 추모 메시지를 추출하여 Firestore로 이전
 */
 ...
 * 마이그레이션 실행
 */
export const migrateLegacyMessages = async (): Promise<void> => {
  try {
    console.log('성남시 추모 메시지 마이그레이션 시작...');

    // 이미 마이그레이션되었는지 확인
    const existingMessages = await getDocs(collection(db, 'memorial_messages'));
    const legacyMessages = existingMessages.docs.filter(doc =>
      doc.data().isLegacy === true
    );

    if (legacyMessages.length > 0) {
      console.log('이미 마이그레이션된 메시지가 있습니다.');
      return;
    }
